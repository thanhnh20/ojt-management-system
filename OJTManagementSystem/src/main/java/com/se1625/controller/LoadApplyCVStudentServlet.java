 ...
 */
public class LoadApplyCVStudentServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        //load in4 sinh viên
        //1. lay session và check attribute 
        HttpSession session = request.getSession(false);
        ServletContext context = this.getServletContext();
        Properties properties = (Properties) context.getAttribute("SITE_MAPS");
        String url = properties.getProperty(MyApplicationConstants.LoadApplyCVStudentFeature.HOME_AFTER_CLICK1_PAGE);
        try {
            //if (session != null) {
            ...
            log("SQLException occurs LoadApplyCVStudentServlet " + ex.getMessage());
        } catch (NamingException ex) {
            log("NamingException occurs LoadApplyCVStudentServlet " + ex.getMessage());
        } finally {
